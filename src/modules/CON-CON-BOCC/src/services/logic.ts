...
export function calculateHead(dInt: number, config: HeadConfig): HeadCalculated {
  // === Testa conica (fondo TRONCOCONICO oppure coperchio conico, con raccordo cono/colletto) ===
  if (config.type === 'conico') {
    ...
    // (con r_min = 0 si ricade nel cono a punta di CON-CON)
...
export function calculateTank(input: TankInput): CalculationResult {
  ...
  // diametro dBocchello e altezza hBocchello, che diventa la nuova quota 0.
  ...
  // senza bocchello z0 = 0 e si ricade nel comportamento di CON-CON).
  ...
  for (let h = 1; h <= H_tot; h++) {
    ...
    if (h <= z0) {
      ...
      if (isConicFondo) {
        // Tronco di cono retto: raggio lineare da r_min (base minore, a h_zona=0) fino a Y (a h_zona=H_cono)
