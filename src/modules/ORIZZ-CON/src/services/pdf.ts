...
export async function generateCalibrationPDF(
  ...
  try {
    ...
    const listData: { cm: number; mm: number; litri: number; delta: number; pct: number }[] = [];
    for (let cm = 0; cm <= maxCm; cm++) {
      ...
      listData.push({
        ...
        mm,
        litri: litriVal,
        pct: result.volumeTotale > 0 ? (litriVal / result.volumeTotale) * 100 : 0,
    ...
    const labels: Record<Language, any> = {
      it: {
        ...
        cylinderLength: 'Lunghezza Cilindrica (L_cil):',
        totalHeight: 'Livello Massimo (H_tot):',
        density: 'Densità del Fluido (rho):',
        
        // Volumes
        volumeTitle: 'Volumi dei Singoli Componenti',
        bottomVolume: 'Volume Testa Sinistra Conica:',
        cylinderVolume: 'Volume Mantello Cilindrico:',
        topVolume: 'Volume Testa Destra Conica:',
        totalVolume: 'VOLUME TOTALE NOMINALE:',
        
        // Sheets
        sheetTitle: 'Dati Costruttivi e Lamiere (Acciaio)',
        bottomHead: 'Testa Sinistra Conica:',
        topHead: 'Testa Destra Conica:',
        ...
        weightPerCm: 'Peso per cm di lunghezza cilindro:',
      ...
      en: {
        ...
        weightPerCm: 'Weight per cm of cylinder length:',
      ...
      es: {
        ...
        cylinderLength: 'Longitud Cilíndrica (L_cil):',
        totalHeight: 'Nivel Máximo (H_tot):',
        density: 'Densidad del Fluido (rho):',
        
        volumeTitle: 'Volúmenes de los Componentes Individuales',
        bottomVolume: 'Volumen Cabezal Izquierdo Cónico:',
        cylinderVolume: 'Volumen del Cuerpo Cilíndrico:',
        topVolume: 'Volumen Cabezal Derecho Cónico:',
        totalVolume: 'VOLUMEN NOMINAL TOTAL:',
        
        sheetTitle: 'Detalles de Fabricación y Chapa (Acero)',
        bottomHead: 'Cabezal Izquierdo Cónico:',
        topHead: 'Cabezal Derecho Cónico:',
        ...
        weightPerCm: 'Peso por cm de longitud de cilindro:',
      ...
      de: {
        ...
        weightPerCm: 'Gewicht pro cm Zylinderlänge:',
    ...
    // Tank Shape Clue/Schematic on the Top Right — serbatoio ORIZZONTALE con teste coniche
    const xApexL = 165;
    const xCylL = 173;
    const xCylR = 191;
    const xApexR = 199;
    ...
    doc.rect(xCylL, yAx - hHalf, xCylR - xCylL, hHalf * 2, 'F');
    doc.triangle(xCylL, yAx - hHalf, xCylL, yAx + hHalf, xApexL, yAx, 'F');
    doc.triangle(xCylR, yAx - hHalf, xCylR, yAx + hHalf, xApexR, yAx, 'F');

    doc.setDrawColor(6, 78, 59);
    doc.setLineWidth(0.4);
    doc.line(xCylL, yAx - hHalf, xCylR, yAx - hHalf);
    doc.line(xCylL, yAx + hHalf, xCylR, yAx + hHalf);
    doc.line(xCylL, yAx - hHalf, xApexL, yAx);
    doc.line(xCylL, yAx + hHalf, xApexL, yAx);
    doc.line(xCylR, yAx - hHalf, xApexR, yAx);
    doc.line(xCylR, yAx + hHalf, xApexR, yAx);

    // giunzioni (saldature) cono/cilindro
    doc.setDrawColor(110, 160, 140);
    doc.setLineWidth(0.2);
    doc.line(xCylL, yAx - hHalf, xCylL, yAx + hHalf);
    doc.line(xCylR, yAx - hHalf, xCylR, yAx + hHalf);

    // asse di simmetria (tratteggiato orizzontale)
    ...
    doc.setLineWidth(0.15);
    {
      ...
      let cx = xApexL - 2;
      while (cx < xApexR + 2) {
        const nx = Math.min(cx + dash, xApexR + 2);
        doc.line(cx, yAx, nx, yAx);
        cx = nx + gap;
      }
    }

    // etichette sotto la sagoma
    const labelLeft = lang === 'en' ? 'Left head' : lang === 'es' ? 'Cab. izq.' : lang === 'de' ? 'Boden links' : 'Testa sx';
    const labelMid = lang === 'en' ? 'Cylinder' : lang === 'es' ? 'Cilindro' : lang === 'de' ? 'Zylinder' : 'Cilindro';
    const labelRight = lang === 'en' ? 'Right head' : lang === 'es' ? 'Cab. der.' : lang === 'de' ? 'Boden rechts' : 'Testa dx';
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(5.5);
    doc.setTextColor(107, 114, 128);
    doc.text(labelLeft, xApexL + 3, yAx + hHalf + 5, { align: 'center' });
    doc.text(labelMid, (xCylL + xCylR) / 2, yAx + hHalf + 5, { align: 'center' });
    doc.text(labelRight, xApexR - 3, yAx + hHalf + 5, { align: 'center' });
    ...
    const grpTop = lang === 'en' ? 'right conical head' : lang === 'es' ? 'cabezal derecho cónico' : lang === 'de' ? 'rechter konischer Boden' : 'testa destra conica';
    const grpCyl = lang === 'en' ? 'cylindrical section' : lang === 'es' ? 'sección cilíndrica' : lang === 'de' ? 'Zylinderteil' : 'sezione cilindrica';
    const grpCon = lang === 'en' ? 'left conical head' : lang === 'es' ? 'cabezal izquierdo cónico' : lang === 'de' ? 'linker konischer Boden' : 'testa sinistra conica';
    const grpAll = lang === 'en' ? 'Left head + cylindrical part + right head' : lang === 'es' ? 'Cabezal izq. + parte cilíndrica + cabezal der.' : lang === 'de' ? 'Linker Boden + Zylinderteil + rechter Boden' : 'Testa sinistra + parte cilindrica + testa destra';
    ...
    const lblHcono = lang === 'en' ? 'Head Length incl. collar (h_cono) (mm)' : lang === 'es' ? 'Longitud Cabezal con collarín (h_cono) (mm)' : lang === 'de' ? 'Bodenlänge inkl. Kragen (h_cono) (mm)' : 'Lunghezza Testa compresa di colletto (h_cono) (mm)';
    ...
    const lblLtot = lang === 'en' ? 'Internal Axial Length (L_tot)' : lang === 'es' ? 'Longitud Axial Interna (L_tot)' : lang === 'de' ? 'Innere Axiallänge (L_tot)' : 'Lunghezza Assiale Interna (L_tot)';
    const lblTilt = lang === 'en' ? 'Axis Tilt (0 = horizontal)' : lang === 'es' ? 'Inclinación del eje (0 = horizontal)' : lang === 'de' ? 'Achsneigung (0 = waagerecht)' : 'Inclinazione asse (0 = orizzontale)';
    ...
    const body: any[] = [
      ...
      [grpAll, lblLtot, `${result.L_tot} mm`],
      [grpAll, labels[lang].totalHeight.replace(':',''), `${result.H_tot} mm`],
      [grpAll, lblTilt, `${formatNumPDF(result.angolo, 1)} °`],
    ...
    if (condensed) {
      ...
      const spacerCols = [5, 11];
      const blockStartCols = [0, 6, 12];
      const blockEndCols = [4, 10, 16];
      ...
      for (let p = 0; p < pagesNeeded; p++) {
        ...
        for (let r = 0; r < rowsPerPage; r++) {
          ...
          idxs.forEach((idx, b) => {
            ...
            cells.push(row ? `${formatNumPDF(row.pct, 1)}` : '');
        ...
        blockStartCols.forEach((s) => {
          blockColStyles[s] = { halign: 'left', fontStyle: 'bold', cellWidth: 9 };
          blockColStyles[s + 1] = { halign: 'center', cellWidth: 11 };
          blockColStyles[s + 2] = { halign: 'right', fontStyle: 'bold', cellWidth: 16 };
          blockColStyles[s + 3] = { halign: 'right', cellWidth: 13 };
          blockColStyles[s + 4] = { halign: 'right', cellWidth: 10 };
        });
        ...
        if (finalY) {
          doc.setDrawColor(16, 185, 129);
          doc.setLineWidth(0.15);
          const blockRanges = [[13.5, 72.5], [75.5, 134.5], [137.5, 196.5]];
    ...
    for (let i = 1; i <= totalPagesCount; i++) {
      ...
      doc.text('ORIZZ-CON Taratura', 105, 287, { align: 'center' });
