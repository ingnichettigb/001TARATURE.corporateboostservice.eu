...
export function calculateTank(input: TankInput): CalculationResult {
  ...
  const L_ax = Math.round(z7); // lunghezza assiale interna totale (mm)
  ...
  const raggioProfile = new Array<number>(L_ax + 1).fill(0); // profilo raggio lungo l'asse (x = 0..L_ax)

  for (let h = 1; h <= L_ax; h++) {
    let rVal = 0;
  ...
  // ===== SERBATOIO ORIZZONTALE (eventualmente inclinato) =====
  // Il profilo r(x) lungo l'asse è quello del motore verticale (x = "h" del CON-CON):
  // fondo = testa sinistra (x=0), coperchio = testa destra (x=L_ax).
  // Ogni sezione perpendicolare all'asse è un cerchio r(x); il liquido a livello h
  // occupa il segmento circolare sotto la corda a quota y0 (vedi segmentArea).
  const angoloDeg = Number.isFinite(input.angolo as number) ? (input.angolo as number) : 0;
  const theta = (Math.max(-60, Math.min(60, angoloDeg)) * Math.PI) / 180;
  const sinT = Math.sin(theta);
  const cosT = Math.cos(theta);

  // Estremi verticali del corpo (quota z = x*sin(t) ± r*cos(t)); livello 0 = punto più basso
  let zMin = Infinity;
  let zMax = -Infinity;
  for (let x = 0; x <= L_ax; x++) {
    const lo = x * sinT - raggioProfile[x] * cosT;
    const hi = x * sinT + raggioProfile[x] * cosT;
    if (lo < zMin) zMin = lo;
    if (hi > zMax) zMax = hi;
  }
  ...
  for (let h = 1; h < H_tot; h++) {
    const zPlane = zMin + h; // quota assoluta del pelo libero
    let vol = 0;
    for (let i = 0; i < nSlice; i++) {
      const r = rMid[i];
      if (r <= 0) continue;
  ...
  litriCumulativi[H_tot] = volumeGeomLt; // serbatoio pieno
  // monotonia (protezione da arrotondamenti)
  for (let h = 1; h <= H_tot; h++) {
    if (litriCumulativi[h] < litriCumulativi[h - 1]) litriCumulativi[h] = litriCumulativi[h - 1];
  }
  ...
  const volumeTotale = volumeGeomLt;
