...
import { buildHorizontalTankPlot } from '../utils/tankPlot';
...
export default function App() {
  ...
  const defaultInput: TankInput = {
    ...
    angolo: 0,       // inclinazione asse (gradi, 0 = orizzontale)
    ...
    report: {
      ...
      numeroDisegno: 'ORIZZ-TK-104-REV02',
  ...
  const handleLoadTank = (loadedInput: TankInput, loadedCompilerInfo?: CompilerInfo, tankId?: string) => {
    ...
    const normalized: TankInput = {
      ...loadedInput,
      angolo: Number.isFinite(loadedInput.angolo as number) ? (loadedInput.angolo as number) : 0,
  ...
  // Geometria statica del serbatoio ORIZZONTALE per la vista di stampa
  const printPlot = useMemo(() => buildHorizontalTankPlot(result, 340, 200, 300, 130), [result]);
    ...
    return (
      <div className="min-h-dvh bg-[#ebf2ee] text-neutral-900 font-sans antialiased flex items-center justify-center p-6">
        <div className="bg-white border-4 border-double border-emerald-800 rounded-xl p-8 max-w-md w-full text-center space-y-4 shadow-xs">
          <h1 className="text-xl font-black uppercase text-emerald-950">ORIZZ-CON TARATURA</h1>
      ...
      <div className="hidden print:block p-4 text-neutral-900 bg-white">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-12 gap-6 pt-2">
              ...
              <div className="col-span-5 flex flex-col items-center p-3 border border-neutral-300 rounded-xl bg-neutral-50/40">
                ...
                <svg viewBox="0 0 340 200" className="w-full h-[170px] select-none">
                  <defs>
                    <linearGradient id="tankPrintGrad" x1="0%" y1="0%" x2="0%" y2="100%">
                  ...
                  {/* Level references */}
                  <line x1="8" y1={printPlot.mapLevelToY(0)} x2="332" y2={printPlot.mapLevelToY(0)} stroke="#a3a3a3" strokeWidth="0.75" strokeDasharray="2,2" />
                  <text x="10" y={printPlot.mapLevelToY(0) + 9} className="font-mono text-[8px] font-bold fill-black">0 mm</text>
                  <line x1="8" y1={printPlot.mapLevelToY(result.H_tot)} x2="332" y2={printPlot.mapLevelToY(result.H_tot)} stroke="#a3a3a3" strokeWidth="0.75" strokeDasharray="2,2" />
                  <text x="10" y={printPlot.mapLevelToY(result.H_tot) - 3} className="font-mono text-[8px] font-bold fill-black">{result.H_tot} mm</text>

                  {/* Profile shape */}
                  <path d={printPlot.pathData} fill="url(#tankPrintGrad)" stroke="#4b5563" strokeWidth="1.2" />
                </svg>

                {/* Dimensions summary */}
                <div className="w-full text-[8px] font-mono text-neutral-700 mt-2 space-y-0.5 border-t border-neutral-200 pt-1.5 leading-tight">
                  <div><strong>D_int:</strong> {result.input.dInt} mm</div>
                  <div><strong>L_tot:</strong> {result.L_tot} mm ({lang === 'en' ? 'axial internal length' : lang === 'es' ? 'longitud axial interna' : lang === 'de' ? 'axiale Innenlänge' : 'lunghezza assiale interna'})</div>
                  <div><strong>{t.totalHeight}:</strong> {result.H_tot} mm</div>
                  <div><strong>{t.tiltAngle}:</strong> {formatNum(result.angolo, 1)}°</div>
                </div>
              </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 text-[9.5px] font-mono">
                    ...
                    <div className="bg-neutral-50 p-2.5 rounded-lg border border-neutral-200 space-y-1">
                      ...
                      <div className="flex justify-between border-t border-neutral-200 pt-1 mt-1 text-[8.5px]">
                        ...
                        <span className="font-bold">{formatNum(result.input.coperchio.hCono ?? 0, 1)} mm</span>
                      </div>
                      <div className="flex justify-between text-[8.5px]">
                        <span>
                          {lang === 'en' ? 'Cone Angle:' :
                        ...
                        <span className="font-bold">{formatNum(result.coperchio.alfa, 2)} °</span>
                      </div>
                      <div className="flex justify-between text-[8.5px]">
                        <span>
                          {lang === 'en' ? 'Knuckle Radius (r):' :
        ...
        <div className="p-2 space-y-4 bg-white">
          ...
          <div className="text-[9px] text-neutral-500 font-mono flex justify-between">
            <span>
              {lang === 'en' ? 'Values indicate cumulative liters contained at each centimeter of level.' :
               lang === 'es' ? 'Los valores indican los litros acumulativos contenidos en correspondencia con el centímetro de nivel.' :
               lang === 'de' ? 'Die Werte geben das kumulierte Volumen in Litern pro Zentimeter Füllstand an.' :
               "I valori indicano i litri cumulativi contenuti in corrispondenza del centimetro di livello."}
            </span>
          ...
          <div className="pt-4 flex justify-between items-center text-[8px] font-mono text-neutral-400">
            <span>
              {lang === 'en' ? 'Generated via ORIZZ-CON TARATURA v1.2' :
               lang === 'es' ? 'Generado mediante ORIZZ-CON TARATURA v1.2' :
               lang === 'de' ? 'Generiert über ORIZZ-CON TARATURA v1.2' :
               'Generato tramite ORIZZ-CON TARATURA v1.2'}
            </span>
            <span>
