...
interface CalibrationTableProps {
  ...
  /**
   * Esportazione PDF condivisa con l'header del modulo (ConConApp.handleExportPdf).
...
export default function CalibrationTable({ result, lang = 'it', compilerInfo, onExportPdf, onExportCsvGate, exportsBlocked = false, exportsRemaining = null }: CalibrationTableProps) {
  ...
  const handleExportListCSV = () => {
    ...
    // Localized Headers
    const hHeightCm = lang === 'en' ? 'Level (cm)' : lang === 'es' ? 'Nivel (cm)' : lang === 'de' ? 'Füllstand (cm)' : 'Livello (cm)';
    const hHeightMm = lang === 'en' ? 'Level (mm)' : lang === 'es' ? 'Nivel (mm)' : lang === 'de' ? 'Füllstand (mm)' : 'Livello (mm)';
    ...
    const hPct = t.percentCol;
    csvContent += `${hHeightCm};${hHeightMm};${hVolume};${hDelta};${hPct}\n`;
    
    listData.forEach((row) => {
      csvContent += `${row.cm};${row.mm};${row.litri.toFixed(2)};${row.delta.toFixed(2)};${row.pct.toFixed(2)}\n`;
    });
  ...
  const handleExportGridCSV = () => {
    ...
    const hBaseHeight = lang === 'en' ? 'Base Level' : lang === 'es' ? 'Nivel Base' : lang === 'de' ? 'Basis-Füllstand' : 'Livello Base';
  ...
  // "PDF condensata" nell'header (ConConApp.handleExportPdf), garantendo che
              ...
              <thead>
                <tr className="bg-neutral-100 text-neutral-700 font-bold text-[11px] uppercase border-b border-neutral-200">
                  <th className="py-2.5 px-3 text-left border-r border-neutral-200 bg-neutral-100/50">
                    {lang === 'en' ? 'Base Level' : lang === 'es' ? 'Nivel Base' : lang === 'de' ? 'Basis-Füllstand' : 'Livello Base'}
                  </th>
              ...
              <thead>
                <tr className="bg-neutral-100 text-neutral-700 font-bold text-[11px] uppercase border-b border-neutral-200">
                  <th className="py-3 px-4">
                    {lang === 'en' ? 'Level (cm)' : lang === 'es' ? 'Nivel (cm)' : lang === 'de' ? 'Füllstand (cm)' : 'Livello (cm)'}
                  </th>
                  <th className="py-3 px-4">
                    {lang === 'en' ? 'Level (mm)' : lang === 'es' ? 'Nivel (mm)' : lang === 'de' ? 'Füllstand (mm)' : 'Livello (mm)'}
                  </th>
                  <th className="py-3 px-4">
                  ...
                  <th className="py-3 px-4">{t.percentCol}</th>
                </tr>
              </thead>
                ...
                {paginatedListData.length > 0 ? (
                  paginatedListData.map((row) => (
                    <tr key={row.cm} className="hover:bg-neutral-50 even:bg-neutral-50/20">
                      ...
                      <td className="py-2.5 px-4 text-sky-800 font-semibold">{formatNum(row.pct, 1)} %</td>
                    </tr>
                  ))
                ) : (
                  <tr>
                    <td colSpan={5} className="py-6 px-4 text-center italic text-neutral-400">
