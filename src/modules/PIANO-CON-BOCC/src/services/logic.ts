...
export function calculateHead(dInt: number, config: HeadConfig): HeadCalculated {
  ...
  // PIANO-CON-BOCC: il bocchello di fondo (dBocchello) tronca la punta del
  // cono. Con dBocchello = 0 il calcolo coincide esattamente con quello del
  // cono a punta di BOMB-CON/PIANO-CON (rMin = 0).
  if (config.type === 'conico') {
    const R_base = dInt / 2;
    const r_racc = Math.max(0, config.rRaccordo ?? 30);
    // Raggio della base minore del tronco di cono (mm) = raggio del bocchello.
    // Deve restare all'interno dell'inizio del raccordo: r_min < R_base - r_racc.
    const r_min = Math.max(0, (config.dBocchello ?? 0) / 2);
    ...
    // (con r_min = 0 si ricade nel cono a punta standard)
    ...
    // Disco piano che chiude la base minore (π·r_min²) — assente se non c'è bocchello
...
export function calculateTank(input: TankInput): CalculationResult {
  ...
  // Bocchello cilindrico di fondo (tronchetto sotto la base minore del cono).
  // Il suo diametro viene iniettato nella config del fondo: è ciò che tronca
  // la punta del cono e la trasforma in un tronco di cono (rMin = dBocchello/2).
  ...
  const fondo = calculateHead(dInt, { ...input.fondo, dBocchello });
  const coperchio = calculateHead(dInt, input.coperchio);

  const isConicFondo = input.fondo.type === 'conico';

  // Altezze zone
  const H3_fondo = fondo.H3; // per conico = altezza del tronco di cono puro (sotto il raccordo)
  ...
  // senza bocchello z0 = 0 e si ricade nel comportamento di BOMB-CON/PIANO-CON).
  const z0 = hBocchello;                     // fine bocchello (inizio fondo vero e proprio)
  const z1 = z0 + H3_fondo;                  // fine tronco di cono puro (o calotta) fondo
  ...
  // Precalcolo costanti zona 2 fondo bombato (non usato in PIANO-CON-BOCC, che ha
  // sempre fondo conico, ma mantenuto per coerenza con la firma generica del motore)
  ...
  for (let h = 1; h <= H_tot; h++) {
    ...
    if (h <= z0) {
      ...
      if (isConicFondo) {
        // Tronco di cono retto: raggio lineare da rMin (base minore, a h_zona=0)
        // fino a Y (base maggiore, a h_zona=H_cono). Con rMin=0 è il cono a punta.
