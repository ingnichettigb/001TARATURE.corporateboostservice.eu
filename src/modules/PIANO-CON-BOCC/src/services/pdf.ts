...
export async function generateCalibrationPDF(
  ...
  try {
    ...
    // 1. Fill background: cylindrical body + top flat plate + bottom cone (+ bocchello)
    const coneApexY = y_c + h_c + dome_h + 1.5;
    // Se è presente il bocchello, il cono viene troncato: disegniamo un trapezio
    // (tronco di cono) + un tronchetto rettangolare che sporge in basso, invece
    // della classica punta. Con dBocchello = 0 il codice degenera esattamente
    // nel triangolo a punta originale (nozzleW = nozzleH = 0).
    const hasBocchelloIcon = (result.input.dBocchello ?? 0) > 0;
    const nozzleW = hasBocchelloIcon ? Math.max(1.6, Math.min(4, w_c * 0.22)) : 0;
    const nozzleH = hasBocchelloIcon ? 2.4 : 0;
    const trapBottomY = coneApexY - nozzleH; // vertice del trapezio (inizio tronchetto)
    const halfNozzle = nozzleW / 2;
    ...
    // Bottom cone (conico), troncato dal bocchello quando presente
    const conePts: [number, number][] = [
      [w_c, 0],
      [-(w_c / 2 - halfNozzle), trapBottomY - (y_c + h_c)],
      [0, coneApexY - trapBottomY],
      [-nozzleW, 0],
      [0, trapBottomY - coneApexY],
    ];
    doc.lines(conePts, x_c, y_c + h_c, [1, 1], 'F', true);
    ...
    // Bottom cone outlines (slanted sides, troncati dal bocchello se presente)
    doc.line(x_c, y_c + h_c, x_c + w_c / 2 - halfNozzle, trapBottomY);
    doc.line(x_c + w_c, y_c + h_c, x_c + w_c / 2 + halfNozzle, trapBottomY);
    if (hasBocchelloIcon) {
      doc.line(x_c + w_c / 2 - halfNozzle, trapBottomY, x_c + w_c / 2 - halfNozzle, coneApexY);
      doc.line(x_c + w_c / 2 + halfNozzle, trapBottomY, x_c + w_c / 2 + halfNozzle, coneApexY);
      doc.line(x_c + w_c / 2 - halfNozzle, coneApexY, x_c + w_c / 2 + halfNozzle, coneApexY);
    }
    ...
    const lblRraccTop = lang === 'en' ? 'Knuckle Radius (r_custom) (mm)' : lang === 'es' ? 'Radio Empalme (r_custom) (mm)' : lang === 'de' ? 'Krempenradius (r_custom) (mm)' : 'Raggio di Raccordo (r_custom) (mm)';
    ...
    const body: any[] = [
      ...
      [grpTop, lblRraccTop, `${formatNumPDF(result.coperchio.r, 1)} mm`],
      ...
      // bocchello di fondo (solo se presente: il suo Ø tronca la punta del cono)
    ...
    for (let i = 1; i <= totalPagesCount; i++) {
      ...
      doc.text('PIANO-CON-BOCC Taratura', 105, 287, { align: 'center' });
