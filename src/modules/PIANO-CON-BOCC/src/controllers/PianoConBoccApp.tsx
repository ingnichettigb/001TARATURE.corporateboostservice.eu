...
export default function App() {
  ...
  const defaultInput: TankInput = {
    ...
    dBocchello: 150, // Ø bocchello cilindrico di fondo (mm) — 0 = nessun bocchello (cono a punta)
    ...
    return (
      <div className="min-h-dvh bg-[#ebf2ee] text-neutral-900 font-sans antialiased flex items-center justify-center p-6">
        <div className="bg-white border-4 border-double border-emerald-800 rounded-xl p-8 max-w-md w-full text-center space-y-4 shadow-xs">
          <h1 className="text-xl font-black uppercase text-emerald-950">PIANO-CON-BOCC TARATURA</h1>
      ...
      <div className="hidden print:block p-4 text-neutral-900 bg-white">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-12 gap-6 pt-2">
              ...
              <div className="col-span-5 flex flex-col items-center p-3 border border-neutral-300 rounded-xl bg-neutral-50/40">
                ...
                {/* Legend for the 7 zones (+ zona 0 bocchello, quando presente) */}
                <div className="w-full text-[7.5px] font-mono text-neutral-600 mt-2 space-y-0.5 border-t border-neutral-200 pt-1.5 leading-tight">
                  {(input.dBocchello ?? 0) > 0 && (
                    <div><strong>Zone 0:</strong> {lang === 'en' ? 'Bottom nozzle (cylindrical stub)' : lang === 'es' ? 'Boquilla de fondo (tronco cilíndrico)' : lang === 'de' ? 'Bodenstutzen (zylindrischer Stutzen)' : 'Bocchello di fondo (tronchetto cilindrico)'}</div>
                  )}
                  <div><strong>Zone 1:</strong> {(input.dBocchello ?? 0) > 0 ? (lang === 'en' ? 'Bottom cone (truncated)' : lang === 'es' ? 'Fondo cónico (tronco de cono)' : lang === 'de' ? 'Konusboden (Kegelstumpf)' : 'Fondo conico (tronco di cono)') : (lang === 'en' ? 'Bottom cone (pointed)' : lang === 'es' ? 'Fondo cónico (punta)' : lang === 'de' ? 'Konusboden (spitz)' : 'Fondo conico a punta')}</div>
                  <div><strong>Zone 2:</strong> {lang === 'en' ? 'Bottom knuckle (cone/flange transition)' : lang === 'es' ? 'Raccordo fondo (transición cono/cuello)' : lang === 'de' ? 'Bodenkrempe (Übergang Kegel/Bord)' : 'Raccordo cono/colletto Fondo'}</div>
        ...
        <div className="p-2 space-y-4 bg-white">
          ...
          <div className="pt-4 flex justify-between items-center text-[8px] font-mono text-neutral-400">
            <span>
              {lang === 'en' ? 'Generated via PIANO-CON-BOCC TARATURA v1.2' :
               lang === 'es' ? 'Generado mediante PIANO-CON-BOCC TARATURA v1.2' :
               lang === 'de' ? 'Generiert über PIANO-CON-BOCC TARATURA v1.2' :
               'Generato tramite PIANO-CON-BOCC TARATURA v1.2'}
            </span>
            <span>
