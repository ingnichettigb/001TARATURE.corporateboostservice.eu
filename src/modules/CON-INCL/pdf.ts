...
export async function generateCalibrationPDF(
  ...
  try {
    ...
    // 1. Fill background: cylindrical body + top cone + inclined bottom
    const headExt = dome_h + 1.5;              // estensione grafica di testa (cono sopra / cuneo sotto)
    const coneTopY = y_c - headExt;            // apice del cono (in alto)
    const fondoLowY = y_c + h_c + headExt;     // quota del punto basso del cuneo (lato sinistro, in basso)
    doc.setFillColor(240, 253, 244);
    doc.rect(x_c, y_c, w_c, h_c, 'F');
    // Top cone (conico)
    doc.triangle(x_c, y_c, x_c + w_c, y_c, x_c + w_c / 2, coneTopY, 'F');
    // Bottom wedge (inclinato): punto alto a destra, punto basso a sinistra
    doc.triangle(x_c, y_c + h_c, x_c + w_c, y_c + h_c, x_c, fondoLowY, 'F');

    // 2. Draw tank outlines
    doc.setDrawColor(6, 78, 59);
    doc.setLineWidth(0.4);
    // Left vertical line (fino al punto basso del fondo inclinato)
    doc.line(x_c, y_c, x_c, fondoLowY);
    // Right vertical line
    doc.line(x_c + w_c, y_c, x_c + w_c, y_c + h_c);
    // Top cone outline (two slanted sides)
    doc.line(x_c, y_c, x_c + w_c / 2, coneTopY);
    doc.line(x_c + w_c, y_c, x_c + w_c / 2, coneTopY);
    // Bottom inclined plate (dal punto basso sinistro al punto alto destro)
    doc.line(x_c, fondoLowY, x_c + w_c, y_c + h_c);
    ...
    drawDashedLine(x_c + w_c / 2, coneTopY - 2, x_c + w_c / 2, fondoLowY + 2);
    ...
    const lblHconoCoperchio = lang === 'en' ? 'Cone Height incl. collar (h_cono) (mm)' : lang === 'es' ? 'Altura Cono con collarín (h_cono) (mm)' : lang === 'de' ? 'Konushöhe inkl. Kragen (h_cono) (mm)' : 'Altezza Cono compresa di colletto (h_cono) (mm)';
    const lblDislivelloFondo = lang === 'en' ? 'Slope drop (delta) (mm)' : lang === 'es' ? 'Desnivel (delta) (mm)' : lang === 'de' ? 'Höhenunterschied (delta) (mm)' : 'Dislivello fondo inclinato (delta) (mm)';
    ...
    const body: any[] = [
      ...
      [grpTop, lblHconoCoperchio, `${formatNumPDF(cop.hCono ?? 0, 1)} mm`],
      ...
      // fondo inclinato
      [grpCon, lblDislivelloFondo, `${formatNumPDF(fon.hDislivello ?? 0, 1)} mm`],
    ...
    for (let i = 1; i <= totalPagesCount; i++) {
      ...
      doc.text('CON-INCL Taratura', 105, 287, { align: 'center' });
