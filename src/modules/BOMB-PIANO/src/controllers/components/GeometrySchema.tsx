...
/* ---------- main ---------- */
/*
 * BOMB-PIANO: coperchio BOMBATO (in alto, preset Klöpper/Korbbogen/...) e
 * fondo PIANO (in basso, disco di lamiera con raccordo r, colletto e spessore).
 */
...
export default function GeometrySchema({ input, onChange }: GeometrySchemaProps) {
  ...
  /* --- preset testata bombata (solo coperchio) --- */
  ...
  // coperchio bombato e virola: altezze GRAFICHE FISSE (rappresentative)
  const hCoperchio_px = 96;
  const lCil_px = 250;
  // fondo PIANO: altezza grafica fissa (raccordo + colletto)
  const hFondo_px = 40;
  ...
  // curva del coperchio bombato: peak reale della bezier = 0.75 * rise
  ...
  // profilo: coperchio bombato in alto, virola e fondo PIANO in basso (bordo inferiore rettilineo,
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white border-4 border-double border-emerald-800 rounded-xl p-2 overflow-x-auto">
        ...
        >
          ...
          {/* CATENA DI QUOTE: coperchio bombato + virola + fondo piano */}
          <g>
      ...
      {/* ERRORI GEOMETRICI */}
      {(!geometriaCoperchioValida || !raccordoFondoValido) && (
