 ...
 *
 * BOMB-PIANO: coperchio BOMBATO (testata con calotta + raccordo toroidale) e
 * fondo PIANO (config.type === 'piano': disco di lamiera + raccordo toroidale
 * opzionale di raggio r_custom + colletto).
 */
export function calculateHead(dInt: number, config: HeadConfig): HeadCalculated {
  ...
  // Profilo interno: disco piano di raggio X = R_base - r, raccordo a quarto di toro
  // di raggio r (angolo 90°) fino alla parete, poi colletto cilindrico.
  // r = 0 → disco piano puro (nessun raccordo). Il raggio di raccordo è in config.r_custom.
  if (config.type === 'piano') {
    ...
    return {
      ...
      H2: r_racc,        // zona 2 = raccordo toroidale del fondo piano
      H3: 0,             // zona 1 = calotta: assente nel fondo piano
      Y: X,
      Baric,
      K: r_racc,
...
export function calculateTank(input: TankInput): CalculationResult {
  ...
  const isFlatFondo = input.fondo.type === 'piano';
  ...
  // Con FONDO PIANO la zona 1 (calotta) ha altezza nulla (H3_fondo = 0, z1 = 0);
  // la zona 2 è il raccordo toroidale di raggio r (H2_fondo = r, 0 se non c'è raccordo)
  // e poi segue il colletto del fondo (zona 3).
  ...
  // Precalcolo costanti zona 2 fondo bombato (con fondo piano risultano tutte 0 e non vengono usate)
  ...
  // Fondo piano: l'arco di raccordo parte dal disco piano (h = 0) → nessuno scostamento iniziale
  const BI_fondo = isConicFondo || isFlatFondo ? 0 : fondo.r - Math.sqrt(termSqFondo);
  ...
  for (let h = 1; h <= H_tot; h++) {
    ...
    if (h <= z1) {
      ...
      if (isConicFondo) {
        ...
        // Zona 2 — raccordo toroidale fondo bombato (o fondo piano: quarto di toro da X a R_base)
        const h_zona = h - z1;
      ...
      // Zona 6 — raccordo toroidale coperchio bombato
      const h_zona = h - z5;
