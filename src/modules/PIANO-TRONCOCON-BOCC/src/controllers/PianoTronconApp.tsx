...
export default function App() {
  ...
  const defaultInput: TankInput = {
    ...
    report: {
      ...
      numeroDisegno: 'PIANOTRC-TK-104-REV02',
  ...
  const handleLoadTank = (loadedInput: TankInput, loadedCompilerInfo?: CompilerInfo, tankId?: string) => {
    ...
    // PIANO-TRONCOCON-BOCC: il fondo è per definizione TRONCOCONICO e il coperchio PIANO
    // (con raccordo). Se il file proviene da un'altra tipologia (import "carica
    // comunque" / file legacy) le due testate vengono ricondotte a queste forme.
    ...
    const normalized: TankInput = {
      ...
      // se il coperchio arriva da un'altra tipologia, r_custom era il raggio di
      // una calotta (o di un cono): si azzera (= nessun raccordo, disco piano puro)
      coperchio:
        loadedCoperchio.type === 'piano'
          ? loadedCoperchio
          : { ...loadedCoperchio, type: 'piano', r_custom: 0 },
    };
    setInput(normalized);
    ...
    return (
      <div className="min-h-dvh bg-[#ebf2ee] text-neutral-900 font-sans antialiased flex items-center justify-center p-6">
        <div className="bg-white border-4 border-double border-emerald-800 rounded-xl p-8 max-w-md w-full text-center space-y-4 shadow-xs">
          <h1 className="text-xl font-black uppercase text-emerald-950">PIANO-TRONCOCON-BOCC TARATURA</h1>
      ...
      <div className="hidden print:block p-4 text-neutral-900 bg-white">
            ...
            <div className="grid grid-cols-12 gap-6 pt-2">
              ...
              <div className="col-span-5 flex flex-col items-center p-3 border border-neutral-300 rounded-xl bg-neutral-50/40">
                ...
                <div className="w-full text-[7.5px] font-mono text-neutral-600 mt-2 space-y-0.5 border-t border-neutral-200 pt-1.5 leading-tight">
                  ...
                  <div><strong>Zone 6:</strong> {lang === 'en' ? 'Flat top knuckle (toroidal, h = r)' : lang === 'es' ? 'Raccordo tapa plana (toroidal, h = r)' : lang === 'de' ? 'Krempe Flachdeckel (toroidal, h = r)' : 'Raccordo toroidale Coperchio piano (h = r)'}</div>
                  <div><strong>Zone 7:</strong> {lang === 'en' ? 'Flat top — no dome (h = z6)' : lang === 'es' ? 'Tapa plana — sin cúpula (h = z6)' : lang === 'de' ? 'Flacher Deckel — keine Kuppel (h = z6)' : 'Coperchio piano — nessuna calotta (h = z6)'}</div>
                </div>
              </div>
        ...
        <div className="p-2 space-y-4 bg-white">
          ...
          <div className="pt-4 flex justify-between items-center text-[8px] font-mono text-neutral-400">
            <span>
              {lang === 'en' ? 'Generated via PIANO-TRONCOCON-BOCC TARATURA v1.2' :
               lang === 'es' ? 'Generado mediante PIANO-TRONCOCON-BOCC TARATURA v1.2' :
               lang === 'de' ? 'Generiert über PIANO-TRONCOCON-BOCC TARATURA v1.2' :
               'Generato tramite PIANO-TRONCOCON-BOCC TARATURA v1.2'}
            </span>
            <span>
