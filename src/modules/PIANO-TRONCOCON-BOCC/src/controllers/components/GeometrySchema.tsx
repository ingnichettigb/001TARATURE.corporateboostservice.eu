...
export default function GeometrySchema({ input, onChange }: GeometrySchemaProps) {
  ...
  const hTot = result ? result.H_tot : hConoFondo_calc + hBocc + lCil + hCoperchio_calc;
  ...
  const totalDrawn = hCoperchio_px + lCil_px + hConoBot_px + hBocc_px;
  ...
  const yBaseBotBocc = yBaseBot + hBocc_px; // quota della punta del bocchello (fondo reale)
  ...
  // se r > 0), tronco di cono TRONCOCONICO in basso con base minore piana (più bocchello,
  // se presente, come tronchetto cilindrico sotto la base minore)
  ...
  const boxSumH = 76;
  const box2H = 96;
  const box1H = 262; // fondo troncoconico (in basso, ha anche Ø min. e Ø/H bocchello)
  ...
  return (
    <div className="space-y-4">
        ...
        >
          ...
          {(() => {
            ...
            const yDim = side === 'below'
              ? (hBocc_px > 0 ? yBaseBotBocc + 12 + 40 : yBaseBot + 12)
              : yBaseBot + 10;
          ...
          {hBocc_px > 0 && (() => {
            ...
            const hInputY = Math.max((yBaseBot + yBaseBotBocc) / 2 - 12, coneInputBottom + 2);
            return (
              <g>
                {/* contorno del bocchello già incluso in pathData; qui solo le quote */}
                <line x1={xL} y1={yBaseBotBocc + 2} x2={xL} y2={yDim + 6} stroke="#334155" strokeWidth="1" />
                <line x1={xR} y1={yBaseBotBocc + 2} x2={xR} y2={yDim + 6} stroke="#334155" strokeWidth="1" />
                <line x1={xL} y1={yDim} x2={xR} y2={yDim} stroke="#334155" strokeWidth="1" />
                ...
                <line x1={chainX} y1={yBaseBot} x2={chainX} y2={yBaseBotBocc} stroke="#334155" strokeWidth="1" />
                <line x1={chainX - 7} y1={yBaseBot} x2={chainX + 7} y2={yBaseBot} stroke="#334155" strokeWidth="1" />
                <line x1={chainX - 7} y1={yBaseBotBocc} x2={chainX + 7} y2={yBaseBotBocc} stroke="#334155" strokeWidth="1" />
          ...
          {/* QUOTA TOTALE */}
          <DimLine x={dim4X} y1={yTop} y2={yBaseBotBocc} label={fmt(hTot)} />
      ...
      {/* ERRORI GEOMETRICI */}
      {(raccordoError || baseMinoreError || boccError || !raccordoCoperchioValido) && (
      ...
      <div className="bg-emerald-50/50 border border-emerald-300 rounded-xl p-3">
        ...
        <div className="grid grid-cols-2 gap-y-1 text-xs font-bold text-neutral-800">
          ...
          <span className="text-right font-mono border-t border-emerald-300 pt-1">
            {fmt(hCoperchio_calc + lCil + hConoFondo_calc + hBocc)} mm
          </span>
        ...
        <p
          className={`mt-2 text-xs font-black ${
            Math.abs(hCoperchio_calc + lCil + hConoFondo_calc + hBocc - hTot) <= 1.5
              ? 'text-emerald-800'
              : 'text-rose-800'
          }`}
        >
