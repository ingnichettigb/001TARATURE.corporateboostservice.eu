  ...
  try {
    ...
    const labels: Record<Language, any> = {
      ...
      es: {
        ...
        topVolume: 'Volumen de la Tapa Plana:',
      ...
      de: {
        ...
        topVolume: 'Volumen des flachen Deckels:',
    ...
    // 1. Fill background: cylindrical body + flat top + bottom truncated cone
    const coneApexY = y_c + h_c + dome_h + 1.5;      // quota della base minore (fondo troncoconico)
    const flatTopY = y_c - dome_h - 1.5;             // quota superiore del coperchio piano
    ...
    const flatTopPath = (): [number, number][] => {
      // segmenti relativi (dx, dy) a partire da (x_c, y_c), senso orario (mirror verticale del fondo piano)
    ...
    drawDashedLine(x_c + w_c / 2, flatTopY - 2, x_c + w_c / 2, y_c + h_c + dome_h + 2);

    // 5. Draw text labels and indicator lines
    const labelTop = lang === 'en' ? 'Flat Top' : lang === 'es' ? 'Tapa Plana' : lang === 'de' ? 'Flachdeckel' : 'Coperchio Piano';
    ...
    const grpTop = lang === 'en' ? 'flat top' : lang === 'es' ? 'tapa plana' : lang === 'de' ? 'flacher Deckel' : 'coperchio piano';
    ...
    const lblRcustomTop = lang === 'en' ? 'Knuckle Radius (r_custom) (mm)' : lang === 'es' ? 'Radio Empalme (r_custom) (mm)' : lang === 'de' ? 'Krempenradius (r_custom) (mm)' : 'Raggio di Raccordo (r_custom) (mm)';
    ...
    const body: any[] = [
      ...
      [grpTop, lblRcustomTop, `${formatNumPDF(cop.r_custom ?? 0, 1)} mm`],
      [grpTop, lblAreaDisco, `${formatNumPDF(result.coperchio.Area_disco_da_tagliare_mq, 3)} m²`],
