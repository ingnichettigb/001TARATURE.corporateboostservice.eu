...
export default function GeometrySchema({ input, onChange }: GeometrySchemaProps) {
  ...
  const setDInt = (v: number) => {
    ...
    if (angolo != null) {
      const d = dislivelloFromAngle(angolo, v);
      if (isFinite(d) && d > 0) next.fondo = { ...input.fondo, hDislivello: Math.round(d * 10) / 10 };
    }
    onChange(next);
  };

  // digitando l'ANGOLO si ricalcola il DISLIVELLO
  ...
  // digitando il DISLIVELLO l'angolo viene ricalcolato (derivato, via useMemo)
  ...
  // 3.3 fondo inclinato: unica parte scalata — pendenza proporzionale alla larghezza (Ø)
  const hFondoIdeal = hFondo_calc * scaleBase;
  const maxFondoPx = Math.max(40, availH - hCoperchio_px - lCil_px);
  const hFondo_px = Math.min(Math.max(hFondoIdeal, 24), maxFondoPx);
  ...
  const yApex = yBotHigh + drop_px;         // punto basso del fondo inclinato (lato sinistro)
  ...
  // callout 1 — ancorato al coperchio piano (in alto)
  const callout1X = leftX - 15;
  const callout1Y = yDomeTop + hCoperchio_px / 2;
        ...
        >
          ...
          <g>
            ...
            <line
              ...
              strokeDasharray="3,3"
            />
            <text x={6 + boxW / 2} y={box1Y + 18} textAnchor="middle" fontSize="11" fontWeight="700" fill="#000000">
          ...
          <g>
            ...
            <foreignObject x={chainX + 8} y={(yBotHigh + yApex) / 2 - 12} width="86" height="24">
              <input
                type="number"
                value={dislivello}
