...
/* ---------- main ---------- */
/*
 * CON-PIANO: coperchio CONICO (in alto: cono retto + raccordo cono/colletto + colletto)
 * e fondo PIANO (in basso: disco di lamiera con raccordo r, colletto e spessore).
 */
...
export default function GeometrySchema({ input, onChange }: GeometrySchemaProps) {
  ...
  // FONDO PIANO (in basso)
  const hCollettoFondo = input.fondo.hColletto;
  const rFondo = input.fondo.r_custom ?? 0;
  // fondo piano: unico raggio da verificare è il raccordo r (0 ≤ r ≤ D/2)
  const raccordoFondoValido = rFondo >= 0 && rFondo <= dInt / 2;
  ...
  const hTot = result ? result.H_tot : hCono_calc + lCil + hFondo_calc;
  ...
  const TOP_BAND = 28;   // fascia superiore incomprimibile
  ...
  // virola e fondo PIANO: altezze GRAFICHE FISSE (solo rappresentative)
  const lCil_px = 250;
  const hFondo_px = 40;

  // coperchio conico: unica parte scalata — inclinazione proporzionale alla larghezza (Ø)
  ...
  // ORIENTAMENTO CON-PIANO: cono in ALTO (coperchio), fondo piano in BASSO
  ...
  // raggio grafico del raccordo del fondo piano (proporzionale a r/(D/2), limitato all'altezza del blocco)
  const rFondoValido = Math.min(Math.max(0, rFondo), dInt / 2);
  const cornerPx =
    rFondoValido > 0
      ? Math.min(hFondo_px, Math.max(8, (rFondoValido / (dInt / 2 || 1)) * halfW))
      : 0;

  // profilo: coperchio conico in alto, virola e fondo PIANO in basso (bordo inferiore rettilineo,
  // con angoli raccordati se r > 0)
  const pathData = `
    M ${leftX} ${yCilTop}
    L ${cx} ${yApex}
    L ${rightX} ${yCilTop}
    L ${rightX} ${yBottom - cornerPx}
    ${cornerPx > 0 ? `A ${cornerPx} ${cornerPx} 0 0 1 ${rightX - cornerPx} ${yBottom}` : `L ${rightX} ${yBottom}`}
    L ${leftX + cornerPx} ${yBottom}
    ${cornerPx > 0 ? `A ${cornerPx} ${cornerPx} 0 0 1 ${leftX} ${yBottom - cornerPx}` : `L ${leftX} ${yBottom}`}
    Z
  `;

  // callout 1 — 50% dell'altezza disegnata del cono (in alto a sinistra)
  const coneT = 0.5;
  ...
  // callout 3 — fondo piano (in basso a sinistra, a metà altezza del blocco piano)
  const callout3X = leftX - 15;
  const callout3Y = yCilBot + hFondo_px / 2;
  ...
  const box2H = 96;
  const box3H = 162;
  // riquadro 3 (fondo piano): fisso in basso
  const box3Y = drawH - box3H - 6;
  // riquadro 1 (coperchio conico): fisso in alto
  const box1Y = Math.max(4, Math.min(callout1Y - 40, box3Y - box1H - boxSumH - box2H - 36));
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white border-4 border-double border-emerald-800 rounded-xl p-2 overflow-x-auto">
        ...
        >
          ...
          {/* CATENA DI QUOTE: coperchio conico + virola + fondo piano */}
          <g>
            <line x1={chainX} y1={yApex} x2={chainX} y2={yBottom} stroke="#334155" strokeWidth="1" />
            {[yApex, yCilTop, yCilBot, yBottom].map((yy, i) => (
          ...
          {/* QUOTA TOTALE */}
          <DimLine x={dim4X} y1={yApex} y2={yBottom} label={fmt(hTot)} />
      ...
      {/* ERRORI GEOMETRICI */}
      {(raccordoError || !raccordoFondoValido) && (
        <div className="bg-rose-50 border border-rose-300 rounded-xl p-3 space-y-1">
          ...
          {!raccordoFondoValido && (
            <p className="text-xs font-bold text-rose-900">
              Il raggio di raccordo del fondo piano deve essere compreso tra 0 e metà del diametro interno
              (valore usato nel calcolo: {fmt(Math.min(Math.max(0, rFondo), dInt / 2))} mm).
            </p>
          )}
        </div>
      )}
      ...
      <div className="bg-emerald-50/50 border border-emerald-300 rounded-xl p-3">
        ...
        <div className="grid grid-cols-2 gap-y-1 text-xs font-bold text-neutral-800">
          ...
          <span>Fondo piano (raccordo + colletto)</span>
