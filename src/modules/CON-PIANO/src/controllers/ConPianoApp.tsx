...
export default function App() {
  ...
  const defaultInput: TankInput = {
    ...
    fondo: {
      type: 'piano',   // CON-PIANO: il fondo è sempre un disco piano
      sp: 8,
      hColletto: 40,
      r_custom: 50,    // raggio di raccordo disco/parete (mm); 0 = nessun raccordo
    },
    coperchio: {
      type: 'conico',
      sp: 8,
  ...
  const handleLoadTank = (loadedInput: TankInput, loadedCompilerInfo?: CompilerInfo, tankId?: string) => {
    ...
    // CON-PIANO: il coperchio è per definizione CONICO e il fondo PIANO. Se il file proviene
    // da un'altra tipologia (import "carica comunque" / file legacy) le testate vengono
    // ricondotte a coperchio conico e fondo piano.
    const normalized: TankInput = {
      ...loadedInput,
      spVirola: loadedInput.spVirola && loadedInput.spVirola > 0 ? loadedInput.spVirola : loadedInput.fondo.sp,
      // se il fondo arriva da un'altra tipologia, r_custom era il raggio di una calotta (o di un cono): si azzera
      fondo:
        loadedInput.fondo.type === 'piano'
          ? loadedInput.fondo
          : { ...loadedInput.fondo, type: 'piano', r_custom: 0 },
      // se il coperchio arriva da un'altra tipologia, si usano altezza cono e raccordo di default
      coperchio:
        loadedInput.coperchio.type === 'conico'
          ? loadedInput.coperchio
          : {
              ...loadedInput.coperchio,
              type: 'conico',
              hCono: Math.round(loadedInput.dInt / 2 + loadedInput.coperchio.hColletto),
              rRaccordo: 30,
            },
    };
    setInput(normalized);
    ...
    return (
      <div className="min-h-dvh bg-[#ebf2ee] text-neutral-900 font-sans antialiased flex items-center justify-center p-6">
        <div className="bg-white border-4 border-double border-emerald-800 rounded-xl p-8 max-w-md w-full text-center space-y-4 shadow-xs">
          <h1 className="text-xl font-black uppercase text-emerald-950">CON-PIANO TARATURA</h1>
      ...
      <div className="hidden print:block p-4 text-neutral-900 bg-white">
            ...
            <div className="grid grid-cols-12 gap-6 pt-2">
              ...
              <div className="col-span-5 flex flex-col items-center p-3 border border-neutral-300 rounded-xl bg-neutral-50/40">
                ...
                <svg viewBox="0 0 320 400" className="w-full max-w-[210px] h-[280px] select-none">
                  ...
                  {[
                    { label: 'Z1/Z2', val: result.z1, idx: 0 },
                    { label: 'Z2/Z3', val: result.z2, idx: 1 },
                    { label: 'Z3/Z4', val: result.z3, idx: 2 },
                    { label: 'Z4/Z5', val: result.z4, idx: 3 },
                    { label: 'Z5/Z6', val: result.z5, idx: 4 },
                    { label: 'Z6/Z7', val: result.z6, idx: 5 },
                  ]
                    // fondo piano: le zone 1 e 2 hanno altezza 0 → i separatori coinciderebbero con la base
                    .filter((zone) => zone.val > 0)
                    .map((zone) => {
                    const idx = zone.idx;
                ...
                <div className="w-full text-[7.5px] font-mono text-neutral-600 mt-2 space-y-0.5 border-t border-neutral-200 pt-1.5 leading-tight">
                  <div><strong>Zone 1:</strong> {lang === 'en' ? 'Flat bottom — no dome (h = 0)' : lang === 'es' ? 'Fondo plano — sin cúpula (h = 0)' : lang === 'de' ? 'Flacher Boden — keine Kuppel (h = 0)' : 'Fondo piano — nessuna calotta (h = 0)'}</div>
                  <div><strong>Zone 2:</strong> {lang === 'en' ? 'Flat bottom knuckle (toroidal, h = r)' : lang === 'es' ? 'Raccordo fondo plano (toroidal, h = r)' : lang === 'de' ? 'Krempe Flachboden (toroidal, h = r)' : 'Raccordo toroidale Fondo piano (h = r)'}</div>
                  ...
                  <div><strong>Zone 6:</strong> {lang === 'en' ? 'Top cone/flange knuckle (arc)' : lang === 'es' ? 'Racor cono/collarín tapa (arco)' : lang === 'de' ? 'Deckelverrundung Kegel/Bord (Bogen)' : 'Raccordo cono/colletto Coperchio'}</div>
                  <div><strong>Zone 7:</strong> {lang === 'en' ? 'Top cone (straight)' : lang === 'es' ? 'Cono tapa (recto)' : lang === 'de' ? 'Deckelkegel (gerade)' : 'Cono Coperchio (retto)'}</div>
                </div>
              </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 text-[9.5px] font-mono">
                    <div className="bg-neutral-50 p-2.5 rounded-lg border border-neutral-200 space-y-1">
                      ...
                      <div className="flex justify-between text-[8.5px]">
                        <span>
                          {lang === 'en' ? 'Raw disc area:' :
                           lang === 'es' ? 'Área disco bruto:' :
                           lang === 'de' ? 'Fläche Rohzuschnitt:' :
                           'Area disco grezzo:'}
                        </span>
                        <span className="font-bold">{formatNum(result.fondo.Area_disco_da_tagliare_mq, 3)} m²</span>
                      </div>
                      <div className="flex justify-between text-[8.5px]">
                        <span>
        ...
        <div className="p-2 space-y-4 bg-white">
          ...
          <div className="pt-4 flex justify-between items-center text-[8px] font-mono text-neutral-400">
            <span>
              {lang === 'en' ? 'Generated via CON-PIANO TARATURA v1.2' :
               lang === 'es' ? 'Generado mediante CON-PIANO TARATURA v1.2' :
               lang === 'de' ? 'Generiert über CON-PIANO TARATURA v1.2' :
               'Generato tramite CON-PIANO TARATURA v1.2'}
            </span>
            <span>
