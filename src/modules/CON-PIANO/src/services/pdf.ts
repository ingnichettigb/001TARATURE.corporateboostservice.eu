  ...
  try {
    ...
    const labels: Record<Language, any> = {
      it: {
        ...
        bottomVolume: 'Volume Fondo Piano:',
        ...
        bottomHead: 'Fondo Piano:',
        topHead: 'Coperchio Conico:',
      ...
      en: {
        ...
        bottomVolume: 'Flat Bottom Volume:',
      ...
      es: {
        ...
        bottomVolume: 'Volumen del Fondo Plano:',
      ...
      de: {
        ...
        bottomVolume: 'Volumen des Flachbodens:',
        ...
        bottomHead: 'Flachboden:',
        topHead: 'Konischer Deckel:',
    ...
    // 1. Fill background: cylindrical body + top cone + bottom flat plate
    const coneApexY = y_c - dome_h - 1.5;           // apice cono superiore
    doc.setFillColor(240, 253, 244);
    doc.rect(x_c, y_c, w_c, h_c, 'F');
    // Top cone (conico)
    doc.triangle(x_c, y_c, x_c + w_c, y_c, x_c + w_c / 2, coneApexY, 'F');
    // Bottom flat plate (piano) — angoli inferiori raccordati se il fondo ha raggio di raccordo r > 0
    const fonR = result.fondo.r;
    const cr = fonR > 0 ? Math.min(2.4, Math.max(0.9, (fonR / (result.input.dInt / 2)) * (w_c / 2) * 3)) : 0;
    const flatBottomPath = (): [number, number][] => {
      // segmenti relativi (dx, dy) a partire da (x_c, y_c + h_c), senso orario
      const pts: [number, number][] = [];
      const yb = dome_h;                 // spessore grafico del blocco piano
      const arc = 8;                     // passi per ogni angolo raccordato
      let px = 0, py = 0;
      const push = (x: number, y: number) => { pts.push([x - px, y - py]); px = x; py = y; };
      push(0, yb - cr);
      for (let i = 1; i <= arc && cr > 0; i++) {
        const a = (Math.PI / 2) * (i / arc);
        push(cr - cr * Math.cos(a), yb - cr + cr * Math.sin(a));
      }
      push(w_c - cr, yb);
      for (let i = 1; i <= arc && cr > 0; i++) {
        const a = (Math.PI / 2) * (i / arc);
        push(w_c - cr + cr * Math.sin(a), yb - cr + cr * Math.cos(a));
      }
      push(w_c, 0);
      return pts;
    };
    doc.lines(flatBottomPath(), x_c, y_c + h_c, [1, 1], 'F', true);
    ...
    // Bottom flat plate outline
    doc.lines(flatBottomPath(), x_c, y_c + h_c, [1, 1], 'S', true);
    ...
    const labelBot = lang === 'en' ? 'Flat Bottom' : lang === 'es' ? 'Fondo Plano' : lang === 'de' ? 'Flachboden' : 'Fondo Piano';
    ...
    const grpCon = lang === 'en' ? 'flat bottom' : lang === 'es' ? 'fondo plano' : lang === 'de' ? 'Flachboden' : 'fondo piano';
    ...
    const lblAreaPiano = lang === 'en' ? 'Raw disc area (m²)' : lang === 'es' ? 'Área disco bruto (m²)' : lang === 'de' ? 'Fläche Rohzuschnitt (m²)' : 'Area disco grezzo (m²)';
    ...
    const body: any[] = [
      ...
      // fondo piano
      [grpCon, lblToro, `${formatNumPDF(result.fondo.r, 1)} mm`],
      [grpCon, lblColletto, `${fon.hColletto} mm`],
      [grpCon, lblSviluppo, `${formatNumPDF(result.fondo.Sviluppo_mm, 1)} mm`],
      [grpCon, lblAreaPiano, `${formatNumPDF(result.fondo.Area_disco_da_tagliare_mq, 3)} m²`],
    ...
    for (let i = 1; i <= totalPagesCount; i++) {
      ...
      doc.text('CON-PIANO Taratura', 105, 287, { align: 'center' });
