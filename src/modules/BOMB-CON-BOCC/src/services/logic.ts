...
export function calculateHead(dInt: number, config: HeadConfig): HeadCalculated {
  ...
  // BOMB-CON-BOCC: il cono non termina a punta ma con una base minore piana,
  ...
  if (config.type === 'conico') {
    ...
    // BOMB-CON-BOCC: tronchetto/bocchello cilindrico sotto la base minore.
    // Vincolo: 0 <= r_bocc <= r_min (non può essere più largo della base piccola del cono).
    const r_bocc = Math.min(r_min, Math.max(0, (config.dBocc ?? 0) / 2));
    const h_bocc = r_bocc > 0 ? Math.max(0, config.hBocc ?? 0) : 0;
    ...
    // Tronchetto/bocchello cilindrico (BOMB-CON-BOCC): volume, superficie laterale, disco di fondo
    const V_bocchello_L = (Math.PI * r_bocc * r_bocc * h_bocc) / 1e6;
    const Area_bocchello_laterale_mq = (2 * Math.PI * r_bocc * h_bocc) / 1e6;
    // Disco piano che chiude il fondo del tronchetto (se presente), altrimenti 0
    const Area_bocchello_fondo_mq = h_bocc > 0 ? (Math.PI * r_bocc * r_bocc) / 1e6 : 0;

    // Sviluppo lamiera: tronco di cono (superficie laterale) + fascia raccordo + colletto
    // + disco piano di chiusura (corona circolare se c'è il tronchetto) + tronchetto stesso
    ...
    // Disco piano che chiude la base minore: pieno (π·r_min²) se non c'è tronchetto,
    // corona circolare (π·(r_min²-r_bocc²)) se il tronchetto è più stretto della base,
    // 0 se il tronchetto è largo quanto la base (nessun gradino, il tronco prosegue diretto).
    const Area_fondo_piatto_mq = (Math.PI * (r_min * r_min - r_bocc * r_bocc)) / 1e6;
    ...
    const Area_totale_mq = Area_lat_cono_mq + Area_toro_mq + Area_colletto_mq + Area_fondo_piatto_mq
      + Area_bocchello_laterale_mq + Area_bocchello_fondo_mq;
    ...
    return {
      ...
      Baric,
      K,
      H_esterna_totale: H_totale + config.hColletto + config.sp + h_bocc,
      V_calotta: V_cono_totale_L, // tronco di cono + fascia raccordo (tronco) + spicchio
      V_toro: 0,
      V_raccordo: 0,
      V_colletto: V_colletto_L,
      V_testata_LT: V_cono_totale_L + V_colletto_L + V_bocchello_L,
      Sviluppo_mm,
      Area_fondo_piatto_mq,
      V_bocchello: V_bocchello_L,
      Area_bocchello_laterale_mq,
      Area_bocchello_fondo_mq,
  ...
  return {
    ...
    V_bocchello: 0,
    Area_bocchello_laterale_mq: 0,
    Area_bocchello_fondo_mq: 0,
...
export function calculateTank(input: TankInput): CalculationResult {
  ...
  // Altezze cumulative (quote, in mm, misurate dalla punta del tronchetto = 0;
  // se non c'è tronchetto z0 = 0 e il comportamento è identico a BOMB-TRONCOCON)
  const z0 = fondo.hBocc;                    // fine tronchetto/bocchello fondo
  ...
  for (let h = 1; h <= H_tot; h++) {
    ...
    if (h <= z0) {
      // Zona 0 — tronchetto/bocchello cilindrico sotto la base minore del cono (raggio costante)
      rVal = fondo.rBocc;
    } else if (h <= z1) {
      if (isConicFondo) {
        // Tronco di cono retto: raggio lineare da r_min (base minore, a h=z0) fino a Y (a h=z1)
        const h_zona = h - z0;
