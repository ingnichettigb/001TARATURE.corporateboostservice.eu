...
export async function generateCalibrationPDF(
  ...
  try {
    ...
    const dBoccPdf = Math.min(result.input.fondo.dMin ?? 0, Math.max(0, result.input.fondo.dBocc ?? 0));
    const hasBocc = dBoccPdf > 0 && (result.input.fondo.hBocc ?? 0) > 0;
    ...
    const boccApexY = coneApexY + (hasBocc ? 2.2 : 0); // piccola estensione grafica fissa
    ...
    // Tronchetto/bocchello: piccolo rettangolo sotto la base minore
    if (hasBocc) {
    ...
    if (hasBocc) {
      // flat reducer ring (if narrower) + tronchetto outline
    ...
    const lblDbocc = lang === 'en' ? 'Nozzle Diameter (d_bocc) (mm)' : lang === 'es' ? 'Diámetro Tobera (d_bocc) (mm)' : lang === 'de' ? 'Stutzendurchmesser (d_bocc) (mm)' : 'Diametro Tronchetto (d_bocc) (mm)';
    const lblHbocc = lang === 'en' ? 'Nozzle Height (h_bocc) (mm)' : lang === 'es' ? 'Altura Tobera (h_bocc) (mm)' : lang === 'de' ? 'Stutzenhöhe (h_bocc) (mm)' : 'Altezza Tronchetto (h_bocc) (mm)';
    const lblVolBocc = lang === 'en' ? 'Nozzle Volume' : lang === 'es' ? 'Volumen Tobera' : lang === 'de' ? 'Stutzenvolumen' : 'Volume Tronchetto';
    ...
    const body: any[] = [
      ...
      ...(((fon.dBocc ?? 0) > 0 && (fon.hBocc ?? 0) > 0) ? [
        [grpCon, lblDbocc, `${formatNumPDF(fon.dBocc ?? 0, 1)} mm`],
        [grpCon, lblHbocc, `${formatNumPDF(fon.hBocc ?? 0, 1)} mm`],
        [grpCon, lblVolBocc, `${formatNumPDF(result.fondo.V_bocchello, 2)} l`],
      ] : []),
    ...
    for (let i = 1; i <= totalPagesCount; i++) {
      ...
      doc.text('BOMB-CON-BOCC Taratura', 105, 287, { align: 'center' });
