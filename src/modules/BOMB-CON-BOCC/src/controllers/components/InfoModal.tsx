...
export default function InfoModal({ isOpen, onClose, lang }: InfoModalProps) {
  ...
  const content = {
    it: {
      ...
      intro: "Questo applicativo professionale esegue la taratura geometrica millimetrica di serbatoi cilindrici ad asse orizzontale dotati di fondo troncoconico (cono con base minore piana, di diametro d_min, sotto la quale può essere presente un tronchetto/bocchello cilindrico opzionale di diametro d_bocc ≤ d_min e altezza h_bocc) e coperchio bombato standard (Klöpper, Korbbogen) o completamente personalizzato. Il calcolo volumetrico si basa su una discretizzazione continua a passo di 1 millimetro.",
      ...
      sections: [
        {
          ...
          items: [
            ...
            "**Zona 0 (Tronchetto/bocchello, opzionale)**: se presente (d_bocc > 0), tratto cilindrico di raggio costante d_bocc/2 sotto la base minore del cono; sempre d_bocc ≤ d_min. Se d_bocc = d_min il tronchetto prosegue il cono senza gradino, altrimenti resta un disco piano di riduzione (corona circolare) tra i due.",
            "**Zona 1 (Tronco di cono del fondo)**: Dal fondo piano di diametro d_min (base minore, subito sopra l'eventuale tronchetto) fino all'inizio del raccordo; il raggio cresce linearmente da d_min/2 a Y. Con d_min = 0 il cono termina a punta.",
