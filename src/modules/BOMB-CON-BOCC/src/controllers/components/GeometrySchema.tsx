...
/* ---------- helpers geometria fondo TRONCOCONICO (stessa convenzione del motore) ---------- */
// h_cono = ALTEZZA TOTALE del fondo troncoconico, COLLETTO INCLUSO.
// r_min  = raggio della base minore (Ø base minore / 2). Con r_min = 0 il cono termina a punta.
const hNetFromAngle = (alfaDeg: number, R_base: number, r_racc: number, r_min: number): number => {
...
export default function GeometrySchema({ input, onChange }: GeometrySchemaProps) {
  ...
  const rMin = dMin / 2;
  const hCono = input.fondo.hCono ?? Math.round(Math.max(0, dInt / 2 - rMin) + hCollettoCono);
  // BOMB-CON-BOCC: tronchetto/bocchello cilindrico sotto la base minore (0 <= dBocc <= dMin)
  const dBocc = Math.min(dMin, Math.max(0, input.fondo.dBocc ?? 0));
  const rBocc = dBocc / 2;
  const hBocc = dBocc > 0 ? Math.max(0, input.fondo.hBocc ?? 0) : 0;
  const hCollettoCoperchio = input.coperchio.hColletto;

  const angolo = useMemo(() => {
    const a = angleFromHTot(hCono, dInt / 2, rRaccordoCono, hCollettoCono, rMin);
    return a != null ? Math.round(a * 10) / 10 : null;
  }, [hCono, dInt, rRaccordoCono, hCollettoCono, rMin]);

  // la base minore deve restare all'interno dell'inizio del raccordo (Ø_min < Ø − 2·R racc.)
  const baseMinoreError =
    dMin > 0 && dMin >= dInt - 2 * rRaccordoCono
  ...
  // il tronchetto/bocchello non può essere più largo della base minore del tronco di cono
  const boccholloError =
    dBocc > dMin
      ? 'Il diametro del tronchetto non può essere maggiore del diametro della base minore del tronco di cono (Ø tronchetto ≤ Ø base minore).'
      : null;
  ...
  // il tronchetto non può superare la base minore: se dMin si restringe, dBocc si adegua
  const patchDBocc = (v: number) => patchFondo({ dBocc: Math.max(0, Math.min(dMin, v)) });
  ...
  const setDInt = (v: number) => {
    ...
    if (angolo != null) {
      const h = hTotFromAngle(angolo, v / 2, rRaccordoCono, hCollettoCono, rMin);
  ...
  const setAngolo = (v: number) => {
    if (!(v > 0 && v < 90)) return;
    const h = hTotFromAngle(v, dInt / 2, rRaccordoCono, hCollettoCono, rMin);
  ...
  const DIM_BAND = 36;  // 1.6 fascia quota Ø base minore, sotto il fondo
  ...
  // 3.4 tronchetto/bocchello: piccola estensione grafica scalata (con un minimo visibile)
  const hBoccIdeal = hBocc > 0 ? Math.max(hBocc * scaleBase, 14) : 0;
  const maxConoPx = Math.max(40, availH - hCoperchio_px - lCil_px - hBoccIdeal);
  const hCono_px = Math.min(hConoIdeal, maxConoPx);
  const hBocc_px = hBoccIdeal;
  ...
  const yBottom = yCilBot + hCono_px;         // quota della base minore (fine tronco di cono)
  const yBottomBocc = yBottom + hBocc_px;     // quota della punta del tronchetto (fondo reale)
  ...
  // semi-larghezza grafica del tronchetto (sempre <= rMinPx, come nel vincolo reale)
  ...
  // callout 3 — 50% dell'altezza disegnata del tronco di cono (percentuale, non px assoluti)
  const coneT = 0.5;
  const coneVX = cx - rMinPx - leftX;
  const coneVY = yBottom - yCilBot;
        ...
        >
          ...
          <g>
            ...
            <text x={6 + boxW / 2} y={boxSumY + 18} textAnchor="middle" fontSize="11" fontWeight="600" fill="#000000">
              Sezione cilindrica + troncoconica
            </text>
          ...
          <g>
            ...
            <foreignObject x={12} y={box3Y + 156} width={boxW - 18} height="24">
              <MiniField
                label="H tronch."
                value={hBocc}
                onChange={(v) => patchFondo({ hBocc: Math.max(0, v) })}
                labelWidth="58px"
                width="78px"
              />
            </foreignObject>
          ...
          {/* TRONCHETTO / BOCCHELLO — Ø e altezza (solo se presente) */}
          {hBocc_px > 0 && (() => {
            ...
            return (
              <g>
                {/* contorno tronchetto già incluso in pathData; qui solo le quote */}
                ...
                <line x1={xL} y1={yDim} x2={xR} y2={yDim} stroke="#334155" strokeWidth="1" />
                <text x={cx - 50} y={yDim + 20} textAnchor="end" fontSize="13" fontWeight="700" fill="#000000">Ø tronch.</text>
                <foreignObject x={cx - 46} y={yDim + 4} width="92" height="24">
                  <input
                    ...
                    style={editableDimStyle}
                    className="editable-dim"
                    title="Diametro interno del tronchetto/bocchello (mm) — deve essere ≤ Ø base minore"
                  />
                </foreignObject>
                {/* quota altezza tronchetto, a fianco a destra */}
                <line x1={chainX} y1={yBottom} x2={chainX} y2={yBottomBocc} stroke="#334155" strokeWidth="1" />
                <line key="tb-top" x1={chainX - 7} y1={yBottom} x2={chainX + 7} y2={yBottom} stroke="#334155" strokeWidth="1" />
                <line key="tb-bot" x1={chainX - 7} y1={yBottomBocc} x2={chainX + 7} y2={yBottomBocc} stroke="#334155" strokeWidth="1" />
                <foreignObject x={chainX + 8} y={(yBottom + yBottomBocc) / 2 - 12} width="86" height="24">
                  <input
                    type="number"
                    value={hBocc}
                    onChange={(e) => patchFondo({ hBocc: Math.max(0, Number(e.target.value)) })}
                    style={editableDimStyle}
                    className="editable-dim"
                    title="Altezza del tronchetto/bocchello (mm)"
                  />
                </foreignObject>
              </g>
            );
          ...
          <g>
            ...
            <foreignObject x={chainX + 8} y={(yCilBot + yBottom) / 2 - 12} width="86" height="24">
              <input
                type="number"
                value={hCono}
          ...
          {/* QUOTA TOTALE (include il tronchetto, se presente) */}
          ...
          {(() => {
            ...
            return (
              <g>
                ...
                <line x1={cx + rMinPx} y1={yBottom} x2={rightX} y2={yCilBot} stroke="#94a3b8" strokeWidth="1" strokeDasharray="3,3" />
