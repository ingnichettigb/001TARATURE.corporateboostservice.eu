...
export default function App() {
  ...
  const defaultInput: TankInput = {
    ...
    fondo: {
      ...
      sp: 8,
      hColletto: 40,
      hCono: 900, // altezza totale (tronco di cono + raccordo + colletto)
      rRaccordo: 30, // raccordo cono/colletto (mm)
      dMin: 300, // diametro interno della base minore del tronco di cono (mm)
      dBocc: 100, // diametro interno del tronchetto/bocchello (mm), <= dMin
      hBocc: 80,  // altezza del tronchetto/bocchello (mm)
    },
    coperchio: {
  ...
  const handleLoadTank = (loadedInput: TankInput, loadedCompilerInfo?: CompilerInfo, tankId?: string) => {
    ...
    // BOMB-CON-BOCC: il fondo è per definizione TRONCOCONICO. Se il file proviene da un'altra
    // tipologia (import "carica comunque" / file legacy, es. BOMB-CON a punta) il fondo viene
    // ricondotto a conico e il Ø della base minore, se assente, parte da 0 (= cono a punta).
    const loadedFondo = loadedInput.fondo;
    const dMinLoaded = Math.max(0, loadedFondo.dMin ?? 0);
    // Il tronchetto/bocchello non può superare la base minore del tronco di cono.
    const dBoccLoaded = Math.min(dMinLoaded, Math.max(0, loadedFondo.dBocc ?? 0));
    const hBoccLoaded = dBoccLoaded > 0 ? Math.max(0, loadedFondo.hBocc ?? 0) : 0;
    const normalized: TankInput = {
      ...loadedInput,
      spVirola: loadedInput.spVirola && loadedInput.spVirola > 0 ? loadedInput.spVirola : loadedInput.fondo.sp,
      fondo:
        loadedFondo.type === 'conico'
          ? { ...loadedFondo, dMin: dMinLoaded, dBocc: dBoccLoaded, hBocc: hBoccLoaded }
          : {
              ...loadedFondo,
              type: 'conico',
              rRaccordo: 30,
              ...
              dBocc: 0,
              hBocc: 0,
              hCono: Math.round(loadedInput.dInt / 2 + loadedFondo.hColletto),
            },
    };
    setInput(normalized);
    ...
    return (
      <div className="min-h-dvh bg-[#ebf2ee] text-neutral-900 font-sans antialiased flex items-center justify-center p-6">
        <div className="bg-white border-4 border-double border-emerald-800 rounded-xl p-8 max-w-md w-full text-center space-y-4 shadow-xs">
          <h1 className="text-xl font-black uppercase text-emerald-950">BOMB-CON-BOCC TARATURA</h1>
      ...
      <div className="hidden print:block p-4 text-neutral-900 bg-white">
            ...
            <div className="grid grid-cols-12 gap-6 pt-2">
              ...
              <div className="col-span-5 flex flex-col items-center p-3 border border-neutral-300 rounded-xl bg-neutral-50/40">
                ...
                <div className="w-full text-[7.5px] font-mono text-neutral-600 mt-2 space-y-0.5 border-t border-neutral-200 pt-1.5 leading-tight">
                  <div><strong>Zone 1:</strong> {lang === 'en' ? 'Bottom truncated cone' : lang === 'es' ? 'Tronco de cono fondo' : lang === 'de' ? 'Kegelstumpf Boden' : 'Tronco di cono Fondo'}</div>
                  <div><strong>Zone 2:</strong> {lang === 'en' ? 'Cone/flange knuckle (toroidal)' : lang === 'es' ? 'Raccordo cono/collarín (toroidal)' : lang === 'de' ? 'Krempe Konus/Bord (toroidal)' : 'Raccordo toroidale cono/colletto'}</div>
                  ...
                  <div className="grid grid-cols-2 gap-4 text-[9.5px] font-mono">
                    <div className="bg-neutral-50 p-2.5 rounded-lg border border-neutral-200 space-y-1">
                      ...
                      <div className="flex justify-between border-t border-neutral-200 pt-1 mt-1 text-[8.5px]">
                        <span>
                          {lang === 'en' ? 'Minor Base Diam. (d_min):' :
                           lang === 'es' ? 'Diám. base menor (d_min):' :
                           lang === 'de' ? 'Ø kleine Grundfläche (d_min):' :
                           'Diametro base minore (d_min):'}
                        </span>
                        <span className="font-bold">{formatNum(result.input.fondo.dMin ?? 0, 1)} mm</span>
                      </div>
                      {(result.fondo.hBocc > 0) && (
                        <>
                          <div className="flex justify-between text-[8.5px]">
                            <span>
                              {lang === 'en' ? 'Nozzle Diam. (d_bocc):' :
                               lang === 'es' ? 'Diám. tobera (d_bocc):' :
                               lang === 'de' ? 'Stutzen-Ø (d_bocc):' :
                               'Diametro tronchetto (d_bocc):'}
                            </span>
                            <span className="font-bold">{formatNum(result.fondo.rBocc * 2, 1)} mm</span>
                          </div>
                          <div className="flex justify-between text-[8.5px]">
                            <span>
                              {lang === 'en' ? 'Nozzle Height (h_bocc):' :
                               lang === 'es' ? 'Altura tobera (h_bocc):' :
                               lang === 'de' ? 'Stutzenhöhe (h_bocc):' :
                               'Altezza tronchetto (h_bocc):'}
                            </span>
                            <span className="font-bold">{formatNum(result.fondo.hBocc, 1)} mm</span>
                          </div>
                          <div className="flex justify-between text-[8.5px]">
                            <span>
                              {lang === 'en' ? 'Nozzle Volume:' :
                               lang === 'es' ? 'Volumen tobera:' :
                               lang === 'de' ? 'Stutzenvolumen:' :
                               'Volume tronchetto:'}
                            </span>
                            <span className="font-bold">{formatNum(result.fondo.V_bocchello, 3)} l</span>
                          </div>
                        </>
                      )}
                      <div className="flex justify-between text-[8.5px]">
                        <span>
                          {lang === 'en' ? 'Cone Inclination (α):' :
                           lang === 'es' ? 'Inclinación del cono (α):' :
                           lang === 'de' ? 'Konusneigung (α):' :
                           'Inclinazione cono (α):'}
                        </span>
        ...
        <div className="p-2 space-y-4 bg-white">
          ...
          <div className="pt-4 flex justify-between items-center text-[8px] font-mono text-neutral-400">
            <span>
              {lang === 'en' ? 'Generated via BOMB-CON-BOCC TARATURA v1.2' :
               lang === 'es' ? 'Generado mediante BOMB-CON-BOCC TARATURA v1.2' :
               lang === 'de' ? 'Generiert über BOMB-CON-BOCC TARATURA v1.2' :
               'Generato tramite BOMB-CON-BOCC TARATURA v1.2'}
            </span>
            <span>
