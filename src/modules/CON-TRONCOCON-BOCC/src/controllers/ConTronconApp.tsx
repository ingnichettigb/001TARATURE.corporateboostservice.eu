...
export default function App() {
  ...
  const defaultInput: TankInput = {
    ...
    coperchio: {
      ...
      sp: 8,
      hColletto: 40,
      hCono: 900, // altezza totale (cono puro + raccordo + colletto)
      rRaccordo: 30, // raccordo cono/colletto (mm)
      dMin: 0, // il coperchio è un cono a punta: base minore sempre nulla
    },
    report: {
      cliente: 'Petrolchimica Padana S.p.A.',
      riferimento: 'Parco Serbatoi - Area Stoccaggio Sud',
      nomeSerbatoio: 'Serbatoio Diesel TK-104',
      numeroDisegno: 'CONTRC-TK-104-REV02',
  ...
  const handleLoadTank = (loadedInput: TankInput, loadedCompilerInfo?: CompilerInfo, tankId?: string) => {
    ...
    // CON-TRONCOCON-BOCC: il fondo è per definizione TRONCOCONICO e il coperchio CONICO.
    // Se il file proviene da un'altra tipologia (import "carica comunque" / file legacy,
    // es. CON-CON a punta) le due testate vengono ricondotte a conico e il Ø della base
    // minore del fondo, se assente, parte da 0 (= cono a punta).
    const loadedFondo = loadedInput.fondo;
    const loadedCoperchio = loadedInput.coperchio;
    ...
    const normalized: TankInput = {
      ...
      coperchio:
        loadedCoperchio.type === 'conico'
          ? { ...loadedCoperchio, dMin: 0 }
          : {
              ...loadedCoperchio,
              type: 'conico',
              rRaccordo: 30,
              dMin: 0,
              hCono: Math.round(loadedInput.dInt / 2 + loadedCoperchio.hColletto),
            },
    };
    setInput(normalized);
    ...
    return (
      <div className="min-h-dvh bg-[#ebf2ee] text-neutral-900 font-sans antialiased flex items-center justify-center p-6">
        <div className="bg-white border-4 border-double border-emerald-800 rounded-xl p-8 max-w-md w-full text-center space-y-4 shadow-xs">
          <h1 className="text-xl font-black uppercase text-emerald-950">CON-TRONCOCON-BOCC TARATURA</h1>
      ...
      <div className="hidden print:block p-4 text-neutral-900 bg-white">
            ...
            <div className="grid grid-cols-12 gap-6 pt-2">
              ...
              <div className="col-span-5 flex flex-col items-center p-3 border border-neutral-300 rounded-xl bg-neutral-50/40">
                ...
                <div className="w-full text-[7.5px] font-mono text-neutral-600 mt-2 space-y-0.5 border-t border-neutral-200 pt-1.5 leading-tight">
                  ...
                  <div><strong>Zone 2:</strong> {lang === 'en' ? 'Cone/flange knuckle (toroidal)' : lang === 'es' ? 'Raccordo cono/collarín (toroidal)' : lang === 'de' ? 'Krempe Konus/Bord (toroidal)' : 'Raccordo toroidale cono/colletto Fondo'}</div>
                  ...
                  <div><strong>Zone 6:</strong> {lang === 'en' ? 'Flange/cone knuckle (toroidal)' : lang === 'es' ? 'Raccordo collarín/cono (toroidal)' : lang === 'de' ? 'Krempe Bord/Konus (toroidal)' : 'Raccordo toroidale colletto/cono Coperchio'}</div>
                  <div><strong>Zone 7:</strong> {lang === 'en' ? 'Top cone' : lang === 'es' ? 'Cono tapa' : lang === 'de' ? 'Konus Deckel' : 'Cono Coperchio'}</div>
                </div>
              </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 text-[9.5px] font-mono">
                    <div className="bg-neutral-50 p-2.5 rounded-lg border border-neutral-200 space-y-1">
                      ...
                      <div className="flex justify-between border-t border-neutral-200 pt-1 mt-1 text-[8.5px]">
                        <span>
                          {lang === 'en' ? 'Truncated Cone Height (h):' :
                           lang === 'es' ? 'Altura tronco de cono (h):' :
                           lang === 'de' ? 'Kegelstumpfhöhe (h):' :
                           'Altezza tronco di cono (h):'}
                        </span>
        ...
        <div className="p-2 space-y-4 bg-white">
          ...
          <div className="pt-4 flex justify-between items-center text-[8px] font-mono text-neutral-400">
            <span>
              {lang === 'en' ? 'Generated via CON-TRONCOCON-BOCC TARATURA v1.2' :
               lang === 'es' ? 'Generado mediante CON-TRONCOCON-BOCC TARATURA v1.2' :
               lang === 'de' ? 'Generiert über CON-TRONCOCON-BOCC TARATURA v1.2' :
               'Generato tramite CON-TRONCOCON-BOCC TARATURA v1.2'}
            </span>
            <span>
