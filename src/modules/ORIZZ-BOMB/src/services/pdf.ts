  ...
  try {
    ...
    const labels: Record<Language, any> = {
      it: {
        ...
        totalHeight: 'Lunghezza Totale Interna (L_tot):',
      ...
      en: {
        ...
        totalHeight: 'Total Internal Length (L_tot):',
      ...
      es: {
        ...
        totalHeight: 'Longitud Interna Total (L_tot):',
      ...
      de: {
        ...
        cylinderLength: 'Zylindrische Länge (L_cil):',
        totalHeight: 'Innere Gesamtlänge (L_tot):',
    ...
    // Tank Shape Clue/Schematic on the Top Right (serbatoio ORIZZONTALE)
    const x_c = 150;   // inizio corpo cilindrico
    const y_c = 15;    // bordo superiore
    const w_c = 34;    // lunghezza corpo
    const h_c = 13;    // diametro
    const dome_w = 5;  // sporgenza testate

    // Helper to draw a semi-ellipse (angoli in radianti, centro cx,cy)
    ...
    const drawDashedLine = (x1: number, y1: number, x2: number, y2: number) => {
      ...
      let cx = x1;
      while (cx < x2) {
        const nx = Math.min(cx + dash, x2);
        doc.line(cx, y1, nx, y2);
        cx = nx + gap;
      }
    };

    // 1. Fill background: cylindrical body + left dome + right dome
    doc.setFillColor(240, 253, 244);
    doc.rect(x_c, y_c, w_c, h_c, 'F');
    doc.ellipse(x_c, y_c + h_c / 2, dome_w, h_c / 2, 'F');
    doc.ellipse(x_c + w_c, y_c + h_c / 2, dome_w, h_c / 2, 'F');
    ...
    // Left dome outline (semi-ellipse rivolta a sinistra)
    drawSemiEllipse(x_c, y_c + h_c / 2, dome_w, h_c / 2, Math.PI / 2, (3 * Math.PI) / 2);
    // Right dome outline (semi-ellipse rivolta a destra)
    drawSemiEllipse(x_c + w_c, y_c + h_c / 2, dome_w, h_c / 2, -Math.PI / 2, Math.PI / 2);

    // 3. Weld junctions (seams) verticali
    doc.setDrawColor(110, 160, 140);
    doc.setLineWidth(0.2);
    doc.line(x_c, y_c, x_c, y_c + h_c);
    doc.line(x_c + w_c, y_c, x_c + w_c, y_c + h_c);

    // 4. Axis of symmetry (horizontal dashed line)
    ...
    doc.setLineWidth(0.15);
    const y_lab = y_c + h_c + 5.5;
    const labelSpots: [number, string][] = [
      [x_c - dome_w / 2, labelLeft],
      [x_c + w_c / 2, labelMid],
      [x_c + w_c + dome_w / 2, labelRight],
    ];
    labelSpots.forEach(([lx, txt]) => {
      doc.line(lx, y_c + h_c + 0.5, lx, y_lab - 2.6);
      doc.text(txt, lx, y_lab, { align: 'center' });
    });
    ...
    const body: any[] = [
      ...
      // fondo bombato
      [grpCon, lblRoggio, `${formatNumPDF(result.fondo.R, 1)} mm`],
    ...
    for (let i = 1; i <= totalPagesCount; i++) {
      ...
      doc.text('ORIZZ-BOMB Taratura', 105, 287, { align: 'center' });
